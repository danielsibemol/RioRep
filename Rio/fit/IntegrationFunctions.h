...
class OneDimIntFunctor
{
	public:
		OneDimIntFunctor ()
		{}
		void SetValues (string &integration_type, int &final_state, double &M, double &s13, vector<double> &res_masses, vector<double> &res_widths, vector< vector<double> > &res_extra_pars, 
				vector<double> &KK_bin_limits,	vector<TComplex> &pwa_coefs, vector<TComplex> &pwa_coefs_prime, int &index1, int &index2){

			_integration_type = integration_type;
			_final_state = final_state;
			_M = M;
			_s13 = s13;
			_res_masses = res_masses;
			_res_widths = res_widths;
			_res_extra_pars = res_extra_pars;
		...
		double operator() (double *s12, double *p) {
			...
			Amplitudes(_final_state, _M, s12[0], _s13, s23, sig_amp, bkg_amp, resonances, bkg_components, _res_masses, _res_widths, _res_extra_pars, _KK_bin_limits, _pwa_coefs, _pwa_coefs_prime);
			double AccInt = UseAcceptance?acceptance(s12[0], _s13):1;
			if (_integration_type.compare("Bkg") == 0 || _integration_type.compare("BinBkg") == 0){
				return bkg_amp[0].Re();
			} else if (_integration_type.compare("SigRe") == 0){
				...
				return AccInt*temp_sig.Re();
			} else if (_integration_type.compare("SigIm") == 0){
				if (_index1 < _index2) temp_sig = sig_amp[0]*sig_amp[1].Conjugate(sig_amp[1]);
				else if(_index1 > _index2) temp_sig = sig_amp[1]*sig_amp[0].Conjugate(sig_amp[0]);
				else temp_sig = sig_amp[0]*sig_amp[0].Conjugate(sig_amp[0]);
				return AccInt*temp_sig.Im();
			} else if(_integration_type.compare("BinSigRe") == 0){
				if (_index1 < _index2) temp_sig = sig_amp[0]*sig_amp[1].Conjugate(sig_amp[1]);
				else if(_index1 > _index2) temp_sig = sig_amp[1]*sig_amp[0].Conjugate(sig_amp[0]);
				else temp_sig = sig_amp[0]*sig_amp[0].Conjugate(sig_amp[0]);
	...
	private:
		...
		int _final_state;
		double _M, _s13;
		vector<double> _res_masses, _res_widths;
		vector< vector<double> > _res_extra_pars;
...
class TwoDimIntFunctor
{
	public:
		...
		void SetValues (OneDimIntFunctor  &OneDimIntObject, int &npoints, double *points, double *weights, string &integration_type, int &final_state, double &M,
				vector<double> res_masses, vector<double> res_widths, vector< vector<double> > &res_extra_pars, vector<double> KK_bin_limits, vector<TComplex> pwa_coefs, 
				vector<TComplex> pwa_coefs_prime, int &index1, int index2 = 0, double Bins12Min = 0, double Bins12Max = 0){
			...
			_final_state = final_state;
			_M = M;
			_res_masses.resize(res_masses.size());
			_res_masses = res_masses;
			_res_widths = res_widths;
			_res_extra_pars = res_extra_pars;
			_KK_bin_limits = KK_bin_limits;
		...
		double operator() (double *s13 , double *p){
			...
			_OneDimIntObject.SetValues(_integration_type, _final_state, _M, s13[0], _res_masses, _res_widths, _res_extra_pars, _KK_bin_limits, _pwa_coefs, _pwa_coefs_prime, _index1, _index2);
	...
	private:
		...
		double m1, m2, m3, s12max, s12min, s12_lower_limit, s12_upper_limit, c1, c2, c3, c4, Integral, Integral1, Integral2, Integral3, s, m1sq, m2sq, m3sq, res_mass_sq, res_width_sq, thin_res_border_min, thin_res_border_max, thin_res_slice_min, thin_res_slice_max, slice_size; 
                int number_of_slices = 1, thin_res_slice;
};
...
class MassIntFunctor
{
	public:
		...
		void SetValues (TwoDimIntFunctor  &TwoDimIntObject, OneDimIntFunctor  &OneDimIntObject, int &npoints, double *points, double *weights, string &integration_type,
				int &final_state, double &Mass_min, double &Mass_max, double &Bkg_par1, double &Bkg_par2, vector<double> &res_masses, vector<double> &res_widths, 
				vector< vector<double> > &res_extra_pars, vector<double> &KK_bin_limits, vector<TComplex> &pwa_coefs, vector<TComplex> &pwa_coefs_prime, int &index1, 
		...
		double operator() (double *M , double *p){
			...
			if (_final_state == 0) {
				...
				cout << "MassIntFunctor - ERROR: Incorrect final state inserted";
				exit(-1);
			}
			...
			_TwoDimIntObject.SetValues(_OneDimIntObject, _npoints, _points, _weights, _integration_type, _final_state, M[0], _res_masses, _res_widths, _res_extra_pars, _KK_bin_limits,
